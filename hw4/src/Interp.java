...
class Interpreter {
  ...
  static class NameBinding extends Binding {
    ...
    public JamVal value() { return (susp == null) ? JamVoid.ONLY : susp.eval(); }
  ...
  static class NeedBinding extends NameBinding {
    ...
    public JamVal value() {
      if (value == null) {  // a legitimate JamVal CANNOT be null
        if (susp == null) return JamVoid.ONLY;   // Variable is not bound yet
  ...
  static final BindingPolicy CALL_BY_VALUE = new BindingPolicy() {
    ...
    public Binding newDummyBinding(Variable var) { return new ValueBinding(var, JamVoid.ONLY); }
  };
...
class Evaluator implements EvalVisitor {
  ...
  public JamVal forLet(Let l) {
    ...
    for(int i = n - 1; i >= 0; i--) {
      bindings[i] = bindingPolicy.newDummyBinding(vars[i]);  // bind var[i] to dummy value JamVoid
